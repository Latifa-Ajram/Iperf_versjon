...
def handle_client(conn,addr,format):
    ...
    while connected:# så lenge det er en connection fra en client
        msg = conn.recv(1000).decode()# her skal vi legge inn hvor mange bits vi kommer til å motta fra clienten, dette er blocking line of code ,som
        ...
        ####printtest
        print("mottar data fra client")  
        #antall_bytes += len(msg)   
        antall_bytes += sys.getsizeof(msg)                               
        if (msg== "exit" ):# dersom meldingen motatt er exit betyr at clienenten er ikke koblet lenger til porten
            ####printtest
            print("client har koblet fra") 
            break
...
# new meassage to the client
        ...
        #####printtest
        print(f"Server sender respons til klienten: {msg}")

        ######her skal sendes melding om aknowledgemnet........

    end_time= time.time()   
    duration = end_time - start_time
    #antall_bytes = duration * 1000000
    if duration > 0:
            if format == "KB":# hvis vi ønsker å representere bW i KB/s
                ...
                #####Printtest
            print(f"Antall byte som overføres: {antall_bytes}")
            print(f"Tid brukt på overføring: {duration} sekunder")
            ...
            print("{} 0.0 - {:.1f} {:.0f} {}Mbps".format(addr[0]+":"+str(addr[1]),duration,antall_bytes,bandwidth))# her bruker vi format-metoden for å vise tid som
...
def server(ip,port,format):
    ...
    while True:
        ...
        print(f"[ACTIVE CONNECTIONS] {threading.active_count()-1}")# teller antall threads= antall klienter som er connected
